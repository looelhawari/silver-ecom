...
export default function CheckoutPage() {
  ...
  const placeOrder = async () => {
    if (!paymentMethodId) return toast.error(t("choosePayment"));
    setSubmitting(true);
    try {
    ...
    return (
      <StorefrontShell>
        <div className="mx-auto max-w-3xl px-4 py-16 text-center">
          ...
          <Button asChild className="mt-6"><Link href="/shop">{common("shopSilver")}</Link></Button>
        </div>
      </StorefrontShell>
    );
  }
      ...
      <div className="mx-auto grid w-full max-w-6xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[1fr_360px] lg:px-8">
        <form className="space-y-8" onSubmit={(e) => { e.preventDefault(); placeOrder(); }}>
          ...
          <fieldset className="space-y-3 rounded-xl border border-[var(--border)] bg-white p-5">
            ...
            <input className={field} placeholder={t("fullName")} required value={form.customer_name} onChange={set("customer_name")} />
            <div className="grid gap-3 sm:grid-cols-2">
              <input className={field} placeholder={t("phone")} required value={form.customer_phone} onChange={set("customer_phone")} />
              <input className={field} placeholder={t("whatsapp")} value={form.customer_whatsapp} onChange={set("customer_whatsapp")} />
            </div>
            <input className={field} type="email" placeholder={t("email")} value={form.customer_email} onChange={set("customer_email")} />
          </fieldset>

          <fieldset className="space-y-3 rounded-xl border border-[var(--border)] bg-white p-5">
            <legend className="px-1 font-semibold">{t("shippingAddress")}</legend>
            <input className={field} placeholder={t("streetAddress")} required value={form.address_line} onChange={set("address_line")} />
            <div className="grid gap-3 sm:grid-cols-2">
              <input className={field} placeholder={t("city")} required value={form.city} onChange={set("city")} />
              <input className={field} placeholder={t("area")} value={form.area} onChange={set("area")} />
            </div>
            <div className="grid gap-3 sm:grid-cols-3">
              <input className={field} placeholder={t("building")} value={form.building} onChange={set("building")} />
              <input className={field} placeholder={t("floor")} value={form.floor} onChange={set("floor")} />
              <input className={field} placeholder={t("apartment")} value={form.apartment} onChange={set("apartment")} />
            </div>
            <textarea className={field} rows={2} placeholder={t("notes")} value={form.notes} onChange={set("notes")} />
          </fieldset>

          <fieldset className="space-y-2 rounded-xl border border-[var(--border)] bg-white p-5">
            <legend className="px-1 font-semibold">{t("paymentMethod")}</legend>
            {methods?.map((m) => (
            ...
            {selectedMethod?.requires_proof && (
              <p className="text-xs text-[var(--accent)]">{t("paymentProofNote")}</p>
            )}
          </fieldset>
        </form>
          ...
          <ul className="mt-4 space-y-2 text-sm">
            {items.map((i) => (
              <li key={`${i.productId}-${i.variantId ?? "x"}`} className="flex justify-between gap-2">
                ...
                <span>{formatPrice(i.price * i.quantity, "EGP", locale)}</span>
              </li>
            ))}
          </ul>
          <div className="mt-4 space-y-1 border-t border-[var(--border)] pt-4 text-sm">
            <div className="flex justify-between"><span className="text-[var(--muted-foreground)]">{t("subtotal")}</span><span>{totals ? formatPrice(totals.subtotal, totals.currency, locale) : "…"}</span></div>
            <div className="flex justify-between"><span className="text-[var(--muted-foreground)]">{t("shipping")}</span><span>{totals ? formatPrice(totals.shipping_cost, totals.currency, locale) : "…"}</span></div>
            <div className="flex justify-between pt-1 text-base font-semibold"><span>{t("total")}</span><span>{totals ? formatPrice(totals.total, totals.currency, locale) : "…"}</span></div>
          </div>
          <Button className="mt-5 w-full" size="lg" disabled={submitting} onClick={placeOrder}>
            {submitting ? t("placingOrder") : t("placeOrder")}
          </Button>
          <p className="mt-3 text-center text-xs text-[var(--muted-foreground)]">{t("serverTotals")}</p>
        </aside>
      </div>
    </StorefrontShell>
  );
