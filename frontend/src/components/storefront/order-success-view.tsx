  ...
  return (
    <div className="mx-auto w-full max-w-2xl px-4 py-14 text-center sm:px-6">
      ...
      <p className="mt-2 text-[var(--muted-foreground)]">
        Your order code is <span className="font-semibold text-[var(--foreground)]">{code}</span>. Keep it to track your order.
      </p>

      {isLoading && <p className="mt-8 text-sm text-[var(--muted-foreground)]">Loading order…</p>}
        ...
        <div className="mt-8 space-y-4 text-left">
            ...
            <ul className="mt-3 space-y-1 text-sm">
              {data.items.map((i, idx) => (
                <li key={idx} className="flex justify-between">
                  ...
                  <span>{formatPrice(i.line_total, data.currency)}</span>
                </li>
              ))}
            </ul>
          ...
          {data.payment_method && (data.payment_method.instructions || data.payment_method.account_details) && (
            <div className="rounded-xl border border-[var(--accent)]/40 bg-[var(--surface)] p-5">
              <h2 className="font-semibold">Payment — {data.payment_method.name}</h2>
      ...
      <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
        <Button asChild><Link href={`/track-order?code=${code}&phone=${encodeURIComponent(phone)}`}>Track my order</Link></Button>
